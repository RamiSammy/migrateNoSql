...
const createNewUser = async (req, res) => {
  ...
  if (!name || !email || !isNumber.test(String(age)) || !country) {
    ...
    if (added === undefined) {
      res
        .status(400)
        .json({ message: `ERROR: Al agregar un usuario a la base de datos` });
    } else {
      res
        .status(202)
        .json({ message: "Usuario creado correctamente!!", data: added });
    }
  }
};
...
const getAllUsers = async (req, res) => {
  ...
  if (obtained === undefined) {
    res
      .status(400)
      .json({ message: `ERROR: Al obtener usuarios de la base de datos` });
  } else {
    res
      .status(200)
      .json({ message: "Usuarios obtenidos con éxito!", data: obtained });
  }
};
...
const getOneUser = async (req, res) => {
  ...
  if (!id) {
    ...
    if (obtained !== undefined) {
      res
        .status(202)
        .json({ message: "Usuario obtenido con éxito!", data: obtained });
    } else {
      res
        .status(400)
        .json({ message: "ERROR: Al obtener un usuario en la base de datos" });
    }
  }
};
