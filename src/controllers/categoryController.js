...
const createNewCategory = async (req, res) => {
  ...
  if (!name) {
    ...
    if (created !== undefined) {
      ...
      res
        .status(400)
        .json({ message: "ERROR: Al crear la categoría en la base de datos" });
    }
  }
};
...
const readCategories = async (req, res) => {
  ...
  if (obtained !== undefined) {
    ...
    res
      .status(400)
      .json({
        message: "ERROR: Al obtener las categorías en la base de datos",
      });
  }
};
  ...
  if (!name || !isNumber.test(id)) {
    ...
    if (updated !== undefined) {
      res
        .status(202)
        .json({
          message: "Se editó la categoría correctamente!",
          data: updated,
        });
    } else {
      res
        .status(400)
        .json({ message: "ERROR: Al editar la categoría en la base de datos" });
    }
  }
};
...
const getOneCategory = async (req, res) => {
  ...
  if (obtained !== undefined) {
    ...
    res
      .status(400)
      .json({ message: "ERROR: Al obtener la categoria en la base de datos" });
  }
};
